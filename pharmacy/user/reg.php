                                ...
                                <div class="row">
                                    <div class="col-md-3">
                                        <div class="controls">
                                            ...
                                            <?php
                                            $sql = "SELECT rno FROM registration ORDER BY id DESC LIMIT 1";
                                            $stmt = mysqli_query($conn, $sql);

                                            if ($stmt === false) {
                                                die("Error executing query: " . mysqli_error($conn));
                                            } else {
                                                $next_rno = "MR000001"; // Default starting registration number
                                                ...
                                                if (mysqli_num_rows($stmt) > 0) {
                                                    ...
                                                    if (!empty($last_rno)) {
                                                        $last_number = intval(substr($last_rno, 2)); // Extract the numeric part after 'MR'
                                                        $next_number = $last_number + 1;
                                                        $next_rno = "MR" . str_pad($next_number, 6, "0", STR_PAD_LEFT); // Format the next number with leading zeros
                                                    }
                                                }
                                            }
                                            ?>
...
<script>
    ...
    function getDeptDoctors(dept) {
        $.ajax({
            url: "../admin/load/doc_fetch_by_dept.php",
            type: "POST",
            data: { dept: dept },
            dataType: "json",
            success: function (data) {
    ...
    function getDocname(docName) {
        ...
        $.ajax({
            url: "../admin/load/doc_fetch_price.php",
            type: "POST",
    ...
    function checkPhoneNumber() {
        ...
        xhttp.open("GET", "../admin/load/checkPhoneNumber.php?phoneNumber=" + phoneNumber, true);
        xhttp.send();
    }
</script>
